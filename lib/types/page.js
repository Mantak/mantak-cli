...
export function generatePage(name) {
  ...
  const outputPath = `imports/ui/views/pages/${fileName}.js;`;
  if (checkFileExists(outputPath)) {
    logger.exists('vews page: ' + fileName);
    return;
  }
  createFile(`${__dirname}/../../templates/page/page.tt`, `imports/ui/views/pages/${fileName}.js`, {fileName});
  ...
  const options2 = {
    before: { regex: /{path: '\*', component: .*}/ig, last: true},
    asNewLine: false
  };
  const mainFilePath = './imports/ui/views/routes.js';
...
export function destroyPage(name) {
  const fileName = _.snakeCase(name);
  const componentName = _.capitalize(_.camelCase(name));
  const filePath = `./imports/ui/views/pages/${fileName}.js`;
