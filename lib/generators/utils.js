 ...
 * 获取生成文件的位置
 */
export function getOutputPath(type, name) {
  let outputFileName = `${_.snakeCase(name)}.js`;
 ...
 * 根据命令生成文件
 */
export function _generate(type, name) {
  const templateContent = readTemplateContent(type);
  const outputPath = getOutputPath(type, name);
  const templateVariables = getTemplateVaraibles(type, name);
  const component = compileTemplate(templateContent, templateVariables);
